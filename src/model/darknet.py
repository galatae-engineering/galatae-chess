...
class Darknet:
  ...
  def __init__(self):
    ...
    # Si la sortie est un tableau NumPy (tableau de type ndarray)
    if isinstance(out_layers, np.ndarray):
      self.__out_layers = [self.__out_layers[i - 1] for i in out_layers.flatten()]
    else:
      self.__out_layers = [self.__out_layers[i[0] - 1] for i in out_layers]
